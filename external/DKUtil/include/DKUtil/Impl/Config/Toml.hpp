				...
				if (!table.is_table()) {
					...
					for (auto& [key, value] : _manager) {
						...
						case DataType::kDouble:
							{
								double input = data->As<double>()->get_data();
								if (raw->second.is_array() && raw->second.as_array()) {
									if (raw->second.as_array()->size() == 1) {
										if (raw->second.as_array()->front().as_floating_point()) {
											input = raw->second.as_array()->front().as_floating_point()->get();
										} else if (raw->second.as_array()->front().as_integer()) {
											input = static_cast<double>(raw->second.as_array()->front().as_integer()->get());
										}
										...
										for (auto& node : *raw->second.as_array()) {
											...
											} else if (node.as_integer()) {
												array.push_back(static_cast<double>(node.as_integer()->get()));
											}
										}
									...
									} else if (raw->second.as_integer()) {
										input = static_cast<double>(raw->second.as_integer()->get());
									} else {
										break;
									}
								}
						...
						case DataType::kInteger:
							{
								std::int64_t input = data->As<std::int64_t>()->get_data();
								if (raw->second.is_array() && raw->second.as_array()) {
									if (raw->second.as_array()->size() == 1) {
										if (raw->second.as_array()->front().as_integer()) {
											input = raw->second.as_array()->front().as_integer()->get();
										} else if (raw->second.as_array()->front().as_floating_point()) {
											input = static_cast<std::int64_t>(raw->second.as_array()->front().as_floating_point()->get());
										}
										...
										for (auto& node : *raw->second.as_array()) {
											...
											} else if (node.as_floating_point()) {
												array.push_back(static_cast<std::int64_t>(node.as_floating_point()->get()));
											}
										}
									...
									} else if (raw->second.as_floating_point()) {
										input = static_cast<std::int64_t>(raw->second.as_floating_point()->get());
									} else {
										break;
									}
								}
